...
const renderMovieLists = (movies) => {
  ...
  movies.forEach((movie) => {
    ...
    >
        ...
        >
          <h5 class="title-wrap text-lg font-bold text-gray-900 dark:text-white" route="/detail/${imdbID}">
           <a href="/" id="detail-link" data-id="${imdbID}" route="/detail/${imdbID}"> ${Title}</a>
          </h5>
          ...
          <div class="flex justify-between">
            ...
            <button ><i id="${imdbID}" class="like-btn fa-regular fa-heart"></i></button>
          </div>
        </div>
...
const fetchMovies = async (title, type, year = "", page = 1) => {
  const API = {
